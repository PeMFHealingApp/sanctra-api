...
# ---------- lightweight /generate-ir (summary only) ----------
...
def generate_ir():
    ...
    def rt60_tilt_by_band(base_rt, bands):
        out = {}
        for f in bands:
            # mild HF reduction: T_band = base_rt * (f/500Hz)^(-0.18)
            ...
            for ny in range(0, 12):
                for nz in range(0, 12):
                    ...
                    if f <= fmax:
                        ...
                        # mode bandwidth from decay time
    ...
    def early_reflections(dims, alpha_avg, n=6):
        L, W, H = dims
        paths = [0.0, 2*L, 2*W, 2*H, 2*math.sqrt(L*L + W*W), 2*math.sqrt(L*L + H*H)][:max(1, n)]
        taps = []
        for d in paths:
    ...
    try:
        ...
        payload = {
            ...
            "ir_tail_sec_reference": float(min(3.0, max(1.0, base_rt))),  # reference only
